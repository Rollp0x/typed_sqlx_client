...
### Added
- `CrudOpsRef` trait now includes a `table_name(&self) -> &'static str` method, allowing retrieval of the table name for custom queries.
- The derive macro automatically implements this method for all tables, returning the value from `#[crud(table = "...")]`.

### Changed
- Bumped dependency on `typed_sqlx_client_macros` to version 0.2.2.
