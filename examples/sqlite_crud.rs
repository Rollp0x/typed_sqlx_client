...
#[crud(table = "user_infos", db = "sqlite")]
pub struct UserInfo {
    #[crud(primary_key)]
    pub id: Option<Uuid>,
    pub name: String,
...
async fn main() {
    ...
    let query = user_info_table.execute_select_only(sql).await.unwrap();
    ...
    if let Some(id_value) = into_of_value.get("id") {
        if let Some(arr) = id_value.as_array() {
            ...
            panic!("Expected id to be an array of u64 values");
        }
    } else {
        panic!("Expected id field to be present in the result");
    }
